...
public class World {
    ...
    public Vector<Node> renderNodes;
    ...
    void makeNodes() {
        ...
        for(int i=0; i<allNodes.length; i++) {
            ...
            allNodes[i] = new Node(mesh, mesh.min, mesh.max);
        }
        ...
        //Combining nodes
        System.out.println("Old nodes count: "+renderNodes.size());
        
        for(int i=0; i<renderNodes.size(); i++) {
            Node node = renderNodes.elementAt(i);
            
            for(int x=0; x<renderNodes.size(); x++) {
                if(i == x) continue;
                Node node2 = renderNodes.elementAt(x);
                
                if(MathUtils.distance(node.min, node.max, node2.min, node2.max) < 400) {
                    Node nodeNew;
                    
                    if(node.mesh == null) {
                        nodeNew = node;
                        node.min.min(node2.min);
                        node.max.max(node2.max);
                        node.childs.add(node2);
                    } else if(node2.mesh == null) {
                        nodeNew = node2;
                        node2.min.min(node.min);
                        node2.max.max(node.max);
                        node2.childs.add(node);
                    } else {
                        Vector3D min = new Vector3D(node.min);
                        Vector3D max = new Vector3D(node.max);
                        min.min(node2.min);
                        max.max(node2.max);
                        nodeNew = new Node(null, min, max);
                        nodeNew.childs.add(node);
                        nodeNew.childs.add(node2);
                    }
                    
                    renderNodes.removeElementAt(i);
                    renderNodes.removeElementAt(x>i?x-1:x);
                    renderNodes.add(nodeNew);
                    if(i > x) i--;
                    i--;
                    break;
                }
        ...
        while(renderNodes.size() == 1) {
            Node child = renderNodes.firstElement();
            renderNodes.removeElementAt(0);
            renderNodes.addAll(child.childs);
        }
        
        System.out.println("New nodes count: "+renderNodes.size());
    }
